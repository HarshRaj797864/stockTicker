...
export const HomePage = () => {
  ...
  // --- STATE 1: LOGGED OUT (Your existing Landing Page) ---
  if (!user) {
    return (
  ...
  // --- STATE 2: LOGGED IN ("At a Glance" Command Center) ---
  return (
    <div className="relative space-y-10 animate-in fade-in duration-500 min-h-[60vh] py-6">
      
      {/* 1. BACKGROUND GLOW (Added here to match the Landing Page vibe) */}
      ...
      <div className="flex items-end justify-between border-b border-white/10 pb-8">
        <div>
          ...
          <p className="text-gray-400 mt-3 font-medium">Here's your market snapshot.</p>
        </div>
        ...
        <div className="hidden md:block text-right">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#A3FFEA]/10 border border-[#A3FFEA]/20">
            ...
            <span className="text-[#A3FFEA] font-bold text-sm tracking-wide">MARKET OPEN</span>
          </div>
        </div>
      </div>

      {/* 3. MINI TICKER CARDS (Glass Effect) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { name: "NIFTY 50", value: "21,456.30", change: "+1.2%", isPositive: true },
          { name: "SENSEX", value: "71,324.80", change: "+0.8%", isPositive: true },
          { name: "NASDAQ", value: "14,890.10", change: "-0.4%", isPositive: false },
        ].map((index) => (
          <div key={index.name} className="relative group overflow-hidden p-6 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md hover:bg-white/10 transition-all cursor-pointer">
            ...
            <div className="text-gray-400 text-sm font-bold tracking-wider">{index.name}</div>
            <div className="text-3xl font-bold text-white mt-2">{index.value}</div>
            <div className={`text-sm mt-1 font-mono font-bold ${index.isPositive ? 'text-[#A3FFEA]' : 'text-red-400'}`}>
              {index.change}
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-12">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between px-1">
            ...
            <Link to="/watchlists" className="text-sm font-bold text-pink-400 hover:text-pink-300 transition-colors">
              VIEW ALL &rarr;
            </Link>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-sm">
            {[
              { symbol: "RELIANCE", price: "2,560.00", change: "+12.00", isPos: true },
              { symbol: "TATASTEEL", price: "134.50", change: "-1.20", isPos: false },
              { symbol: "INFY", price: "1,450.00", change: "+8.40", isPos: true },
            ].map((stock, i) => (
              <div key={stock.symbol} className="flex items-center justify-between p-5 border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors group">
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm shadow-lg ${i === 0 ? 'bg-gradient-to-br from-blue-500 to-blue-700 text-white' : 'bg-gray-800 text-gray-400'}`}>
                    {stock.symbol[0]}
                  </div>
                  <div>
                    <div className="font-bold text-white group-hover:text-pink-400 transition-colors">{stock.symbol}</div>
                    <div className="text-xs text-gray-500 font-bold">NSE</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="font-mono text-white text-lg">₹{stock.price}</div>
                  <div className={`text-xs font-bold ${stock.isPos ? 'text-[#A3FFEA]' : 'text-red-400'}`}>
                    {stock.change}
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-6">
           ...
           <div className="p-6 rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 border border-white/10 shadow-2xl relative overflow-hidden">
              ...
              <h3 className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-4">🚀 Top Gainer</h3>
              <div className="flex items-center justify-between relative z-10">
                <div>
                  <div className="text-2xl font-bold text-white">Zomato</div>
                  <div className="text-xs text-gray-500 font-medium">Food Delivery</div>
                </div>
           ...
           <Link to="/watchlists" className="block group">
            <div className="h-full p-6 rounded-2xl border-2 border-dashed border-gray-700 hover:border-pink-500 hover:bg-pink-500/5 transition-all cursor-pointer flex flex-col items-center justify-center text-center gap-3">
                ...
                <div className="text-gray-300 font-bold group-hover:text-white">Create New Watchlist</div>
            </div>
           </Link>
        </div>
